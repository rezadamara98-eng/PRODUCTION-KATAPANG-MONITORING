...
// Fungsi khusus debug: kembalikan header mentah, hasil pemetaan index,
// dan beberapa baris mentah (belum diproses) supaya bisa dicek manual.
export async function getPlanSewDebugInfo() {
  const tabName = process.env.GOOGLE_SHEET_TAB_PLAN_SEW || "Plan_SEWvsACT";
  const rows = await fetchRange(tabName, undefined, 840125565);
  const [header, ...body] = rows;
  const idx = buildHeaderIndex(header);
