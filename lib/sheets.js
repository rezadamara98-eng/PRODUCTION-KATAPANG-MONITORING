...
async function getWideLineRows(tabName) {
  ...
  return body
    ...
    // Skip baris yang SEMUA kolom line-nya 0/kosong (hari libur atau belum
    // diisi sheet-nya) - kalau tidak, getYesterdayRow bisa kepilih baris kosong
    // ini sebagai "kemarin" dan bikin data yang ditampilkan salah total.
    .filter((r) => r.lines.some((l) => l.value > 0));
}
...
export async function getEffisiensiLineHistory(days) {
  const tabName = process.env.GOOGLE_SHEET_TAB_EFFISIENSI_LINE || "EFFISIENSI LINE";
  const rows = await getWideLineRows(tabName);
  return days ? rows.slice(-days) : rows;
}
