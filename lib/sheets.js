 ...
 */
// Parser tanggal KHUSUS tab DATA ABSENSI - tab ini hasil IMPORTRANGE dari
// spreadsheet lain yang formatnya M/D/YYYY (Bulan/Tanggal/Tahun, gaya
// Amerika), BEDA dari tab lain di spreadsheet utama yang D/M/YYYY. Jangan
// pakai parseTanggalFleksibel (global) untuk tab ini - itu akan salah baca.
// Dibuktikan dari data: baris dengan angka kedua > 12 (mis. "4/16/2026")
// cuma mungkin valid kalau angka kedua = tanggal, bukan bulan.
...
/**
 * Tab "DATA ABSENSI": TANGGAL, ALFA, CUTI, IZIN - satu baris per tanggal
 * (bukan satu baris per jenis absen kayak asumsi lama). Tidak ada kolom
 * Jumlah MP di tab ini, jadi ditampilkan apa adanya (jumlah orang per jenis),
 * bukan persentase.
 */
export async function getAbsensiYesterday(preFetchedRows) {
  ...
  const allRows = body
    ...
    .map((row) => ({
      tanggal: row[0] || "",
      alfa: parseIdNumber(row[1]),
      cuti: parseIdNumber(row[2]),
      izin: parseIdNumber(row[3]),
    }));
  ...
  if (!anchor && allRows.length > 0) anchor = allRows[allRows.length - 1];
  if (!anchor) return [];

  return [
    { jenisAbsen: "Alfa", jumlah: anchor.alfa, tanggal: anchor.tanggal },
    { jenisAbsen: "Cuti", jumlah: anchor.cuti, tanggal: anchor.tanggal },
    { jenisAbsen: "Izin", jumlah: anchor.izin, tanggal: anchor.tanggal },
  ];
}
